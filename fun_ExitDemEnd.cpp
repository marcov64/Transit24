...
EQUATION("ShareWageIncomeTot")
/*
Wage share as a % of total income
*/
...
EQUATION("sAggProductivity")
/*
Smoothed AggProductivity
*/

v[0]=VL("sAggProductivity",1);

v[1]=V("AggProductivity");
...
EQUATION("sUnemploymentRate")
/*
Smoothed UnemploymentRate
*/

v[0]=VL("sUnemploymentRate",1);
...
if(t==1)
 {
  ...
  CYCLE(cur, "LabClass")
  {v[2]++;
  	switch((int)VS(cur, "LCType") )
  	 {
  	  case 1: v[1]+=VLS(cur->hook, "wage",1)*VLS(cur->hook, "NumWorkers",0);
  	  break;
  	  case 2: v[1]+=VLS(cur->hook, "KWage",1)*VLS(cur->hook, "KNbrWorkers",0);
  	  break;
  	  
  	  case 3: v[1]+=VLS(cur->hook, "wageEF",1)*VLS(cur->hook, "NumWorkersEF",0);
  	  break;
     ...
     case 20:
  	  ...
  	  default: INTERACTS(cur, "Shit init Expenditure", v[1]);
  	  break;
  	 }
  }

  END_EQUATION(v[1]); //at the first step do not
 }
v[1]=V("Consumption");
v[2]=V("aEx");
...
/*
Maximum number of workers, used to compute unemployment
*/
v[0]=V("PopulationNumber");
v[1]=V("NbrWorkers");
v[3] = VL("MaxNumWorkers", 1);
END_EQUATION(v[3]);// REMAIN CONSTANT!!!!!!!
...
WRITE("shareTaxGDPnom", (v[0]+v[1]+v[2])/v[3]);
RESULT(v[0]+v[1]+v[2] )
...
if(v[22]>0 || v[52]==1)
 ...
 if(v[52]==1)
  ...
  if(v[3]/v[11] > 0.1)
  {//order new k only if it increases the capacity of more than 10% of exsting capacity
...
EQUATION("GovDemTot")
/*
Total government demand (gd) to be added to consumers' demand. There are several cases listed below according to their priority
- at time t=1 it is a fixed share of GDP
- if the AvExtraPrice > AvPrice x 5 the gd remains constant. Rationale: if demand is inflationary do not increase gd
- if unemployment 'x' > 0.05 increase past gd by coefficient 1+0.01*(x-0.05)/0.95
- otherwise, leave GvDemTot as previous step


*/


v[31] = V("minShareGovGDP");
v[32] = V("maxShareGovGDP");
...
if(t==1)
 END_EQUATION(v[2]*(v[32]+v[31])/2);
v[35]=3; 
v[4] = VL("GovDemTot", 1);
v[5] = VL("GovernmentFund",1);

v[50]=v[2]*0.05/50; //max variation in a week is 5% yearly of change
...
if(v[21]>v[20]*5)
 {v[35]=1;
  v[4]-=v[50];
  v[34]=max(v[4],v[2]*v[31]);
  WRITE("shareGovGDP", v[34]/v[2]);
  END_EQUATION(v[34]);
 }
...
if(v[14]==1)
 { // compute the first tier workers given their productivity and production needs
  ...
  if(V("Waiting")==1)
   ...
   v[31]=0; 
  v[8]=(V("CapitalCapacity")+v[31])*v[30];
  v[9]=min(v[1],v[8]);
...
if(v[6]/v[0]-1 > V("NumWorkersMaxGr") && v[6]>5 )
  {
   v[6]=v[0]*(1+V("NumWorkersMaxGr"));
  }
  ...
  if(v[6]/V("nu")>1 && NEXT==NULL)
   {
    //need to create the next type of workers
    cur1=ADDOBJ_EXS(p->up,"Labor",p);
    WRITES(cur1,"IdLabor",2);
    v[20]=v[6]/v[17];
    WRITELS(cur1,"NumWorkers",0, t-1);
    WRITELS(cur1,"wage",0, t-1);
    cur3=SEARCH_CND("NumClass",2);
...
v[96]=VL("sAggProductivity",1);
v[97]=VL("sUnemploymentRate",1);

v[98] = exp(0.5 + (log(1*v[96])) + (1*(log(1-v[97]) ) ) ); // (target REAL wage = constant + parameter * productivity level + parameter * employment rate). NOTE: The parameter attached to the productivity variable must equal 1, otherwise real wages will not vary in proportion to the productivity rate - which means the labour share/profit share (i.e. inequality) will change through time (so the model will not be able to reach a steady state). This is the standard conflict inflation equation used in SFC models. 

v[99] = v[0] + (0.025*((v[13]*v[98])-v[0])); // NOMINAL min wage = Nominal min wage in previous period + (Speed of adjustment parameter * (Target Nominal Wage - Nominal min wage in previous period). NOTE: this is a standard partial adjustment function, it ensures that part of the difference between the current and target minwage is made up in each period. 
...
v[72]=1.5-v[18]*2;
if(v[72]<0.9) v[72]=0.9; //share of productivity change that unemployment allows to capture
WRITE("shareProdInWage", v[72]);
...
if(abs(v[0]+v[40]-v[85])/max(max(v[0],v[40]),v[85])>0.000001)
...
v[91]=v[90]/v[0];
if(abs(v[35]+v[1]+v[87]-v[0]-v[40]+v[60]+v[61]-v[86] )/max(v[0],v[1])>0.00000001)
 {
